  ...
  override def run: ZIO[Any & ZIOAppArgs & Scope, Any, Any] =
    for {
      ...
      clubName <- ZIO.fromOption(args.headOption).map(ClubUrlName.wrap)
                    .orElseFail(IllegalArgumentException("Usage: MembershipApp <club-url-name> [since until]"))
      _        <- (args.lift(1) match
                    case Some(sinceStr) =>
                      val since = Instant.parse(sinceStr)
                      val until = args.lift(2).map(Instant.parse).getOrElse(Instant.now())
                      report(clubName, since, until)
                    case None =>
          ...
          client.getWithPermit[ApiPlayer](ApiPlayer.getUrl(username)).flatMap { apiPlayer =>
            ...
            dbState.membersByPlayerId.get(playerId) match {
              case Some(state) =>
                ...
                if state.player.status != statusCategory then
                  changeChunks += StatusChange(now, state.player.status)
                ...
                Player.selectId(playerId).flatMap {
                  case Some(_) =>
                    ...
                    PlayerSnapshot.selectIdLatest(playerId).map { latestOpt =>
                      val needsSnapshot = latestOpt.forall(l => l.username != username || l.status != statusCategory || l.title != apiPlayer.title)
      ...
      client.getWithPermit[ApiPlayer](ApiPlayer.getUrl(oldUsername)).foldZIO(
        ...
        apiPlayer => {
          if apiPlayer.playerId != playerId then
            ...
            if state.player.status != statusCategory || state.player.username != apiPlayer.username || state.player.title != apiPlayer.title then
    ...
    membersByPlayer.toList.map { case (playerId, cms) =>
      ...
      cms.foreach { cm =>
        ...
        if cm.since.compareTo(since) >= 0 && cm.since.compareTo(until) <= 0 then
          ...
          val priorMemberships = members.filter(m => m.clubId == clubId && m.playerId == playerId && m.until.isDefined && m.since != cm.since)
          if priorMemberships.nonEmpty then
            ...
            if priorSnaps.nonEmpty then
              changes += JoinedClub(cm.since)
            else
              changes += NewMember(cm.since)
      ...
      snapsInRange.foreach { snap =>
        ...
        previousSnap.foreach { prev =>
          if prev.username != snap.username then
            changes += UsernameChange(snap.since, prev.username)
          if prev.status != snap.status then
            changes += StatusChange(snap.since, prev.status)
        }
      }
