...
object RecruitmentApp extends ZIOAppDefault {
  ...
  private[recruitment] def recruit(clubUrlName: ClubUrlName, configName: String): ZIO[ChessComClient & Transactor, Throwable, RecruitmentRun] =
    for {
      _        <- ensureTables
      ...
      _        <- Club.upsert(club)
      config   <- RecruitmentConfig.select(clubId, configName).flatMap(
                    ZIO.fromOption(_).orElseFail(
  ...
  private[recruitment] def showReport(clubUrlName: ClubUrlName, runIdOpt: Option[String]): ZIO[Transactor, Throwable, Unit] =
    for {
