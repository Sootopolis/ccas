...
object HistoryMemberQuery {
  ...
  def selectClubPlayerIds(clubId: ClubId): ZIO[PostgresClient, SQLException, Set[PlayerId]] =
    connectZIO {
      sql"SELECT player_id FROM history_member_query WHERE club_id = $clubId"
        .query[PlayerId].run().toSet
    }

  def upsert(item: HistoryMemberQuery): ZIO[PostgresClient, SQLException, Int] =
    connectZIO {
  ...
  def deleteClub(clubId: ClubId): ZIO[PostgresClient, SQLException, Int] =
    connectZIO {
