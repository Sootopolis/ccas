...
object ApiResponseCache {
  ...
  /** Retention cleanup: delete cache rows older than `cutoff`, then drop any `api_response_body` rows no longer
    ...
    * can't be observed between the two steps. Returns the number of cache rows deleted. Concurrent invocations are
    * idempotent — PG MVCC lets each one see its own snapshot and later runs either find no rows to delete or see
    * overlapping sets, so the end state is identical.
    *
