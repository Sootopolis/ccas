...
object PlayerTournamentRef {
  ...
  def selectId(playerId: PlayerId): ZIO[PostgresClient, SQLException, Option[PlayerTournamentRef]] =
    connectZIO(repo.findById(playerId))

  def insert(ref: PlayerTournamentRef): ZIO[PostgresClient, SQLException, Int] =
    connectZIO {
