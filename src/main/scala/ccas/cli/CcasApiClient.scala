...
object CcasApiClient {
  def live(baseUrl: String): URIO[Client, CcasApiClient] =
    ZIO.serviceWith[Client](client => CcasApiClientLive(client, baseUrl))
}

final class CcasApiClientLive(client: Client, baseUrl: String) extends CcasApiClient {
  ...
  private def errorMessage(body: String, status: Status): String =
    body.fromJson[ErrorResponse].map(_.error).getOrElse(s"HTTP ${status.code}")
  ...
  private def ensureSuccess(resp: Response): Task[Unit] =
    ...
    else { resp.body.asString.flatMap(s => ZIO.fail(CliError(errorMessage(s, resp.status), 1))) }
