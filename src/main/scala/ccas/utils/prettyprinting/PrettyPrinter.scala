...
object PrettyPrinter extends AutoDerivation[PrettyPrinter] {
  ...
  /** Settings for PrettyPrinter[T].
    * @param nIndent
    *   Indentation length (default: 2).
    * @param maxWidth
    *   Max width for a single-line element (default: 80).
    * @param namedArguments
    *   Whether arguments are to be named (default: true).
    * @param trailingComma
    *   Whether to have a comma after the last element in a multi-line element (default: false).
    * @param ignoreDefault
    *   Whether to omit elements equal to their default values (default: false).
    */
  ...
  override def join[T](caseClass: CaseClass[PrettyPrinter, T]): PrettyPrinter[T] = new PrettyPrinter[T] {
    override protected def getPrettyString(value: T, setting: Setting, className: String): String =
      if (caseClass.isObject || caseClass.params.isEmpty) { caseClass.typeInfo.short }
      else {
  ...
  given nonEmptyTuplePrinter[H: ClassTag, T <: Tuple](using hpp: PrettyPrinter[H], tpp: PrettyPrinter[T])
      : PrettyPrinter[H *: T] with {
    override protected def getPrettyString(value: H *: T, setting: Setting, className: String): String = {
      ...
      if (printedTail == EmptyTuple.toString()) { s"($printedHead)" }
      else { s"($printedHead, ${printedTail.tail}" }
    }
  }

  private def setOverrideSetting[T](prettyPrinter: PrettyPrinter[T], settingOption: Option[Setting]) =
    if (prettyPrinter.overrideSetting == settingOption) { prettyPrinter }
    else {
      new PrettyPrinter[T] {
