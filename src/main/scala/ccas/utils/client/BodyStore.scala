...
object BodyStore {
  ...
  /** Store a body's bytes, returning `false` when the store rejected the write.
    ...
    * these would have to enumerate the bucket, which is what content-addressing exists to avoid — and mitigated
    * with an R2 lifecycle rule on unreferenced keys. Tracked as an input to #199, since these are billed bytes no
    * accounting in this codebase can see.
    */
