...
object TestClubBoardSql extends ZIOSpecDefault {
  ...
  private val seedClubs = for {
    _ <- Club.upsert(Club(ourClubId, Times.t0, ClubSlug("our-club"), "Our Club", None, None))
    _ <- Club.upsert(Club(oppClubId, Times.t0, ClubSlug("opp-club"), "Opponent Club", None, None))
  } yield ()
